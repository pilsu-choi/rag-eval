...
agent = create_deep_agent(
    model=model,
    backend=FilesystemBackend(root_dir="./files/rag", virtual_mode=True),
    # interrupt_on={
...
prompt = """
너는 RAG evaluation을 위한 데이터를 준비하는 에이전트야.

아래의 내용을 참고해서 작업을 수행해.

   - 파일 목록 내의 모든 json 파일을 읽고 1개당 10개의 testcase를 생성해줘.
   - testcase는 question, ground_truth, filename 를 dict 형태로 작성해줘. single-hop과 multi-hop 질문 형태를 섞어서 작성해줘.
